...
> **Hermes 适配说明（2026-08-07）**：
> - 本 skill 位于 `hermes-home\skills\divination\bazi-ziwei\`，calculator 依赖已装（lunar-typescript）
> - 排盘命令在 Hermes 中用绝对路径：`node <skill_dir>\calculator\dist
un-chart.js --year=... `（stdout 纯 JSON，勿 `2>&1`；Windows 下 JSON 写文件用无 BOM 的 UTF-8）
> - **协同**：本 skill 的算法层（Yiqi+enrichBazi，含格局/旺衰/调候）与 mingyu MCP 互补——本 skill 强在**双盘印证 + 水墨 HTML 海报渲染**，纯排盘计算也可直接调 `mcp__mingyu__*`。用户要海报时用本 skill 的 render 流程，要深度证据链时用 mingyu
> - 海报渲染：`node calculator\dist
ender.js --chart=chart.json --analysis=analysis.json --template=..\templates
eport-zonghe-poster.html --output=<name>.html --currentYear=<YYYY>`
